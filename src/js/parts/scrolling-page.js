export default class ScrollingPage {
	...
	setCircle(e) {
		this.currentSlide = Array.prototype.findIndex.call(this.pagginate.children, (item) => e.target == item);;
		this.setSlide();
	}
	...
	setSlide() {
		this.wrapper.style.cssText = `
			transform: translateY(${-100 / this.slides.length * this.currentSlide}%);
		`;
		Array.prototype.forEach.call(this.pagginate.children, (item) => item.classList.remove('pagination-item-active'));
		this.pagginate.children[this.currentSlide].classList.add('pagination-item-active');
	}
}